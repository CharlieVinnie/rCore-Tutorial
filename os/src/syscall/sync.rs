    ...
    let mutex;
    {
        ...
        mutex = Arc::clone(process_inner.mutex_list[mutex_id].as_ref().unwrap());
        {
            ...
            if process_inner.deadlock_detection {
                task_inner.mutex_requesting.add(mutex_id, 1);
            }
        }
        if process_inner.deadlock_detection {
            if process_inner.check_deadlock() {
                let task = current_task().unwrap();
                let mut task_inner = task.inner_exclusive_access();
                task_inner.mutex_requesting.add(mutex_id, -1);
                return -0xDEAD;
...
pub fn sys_mutex_unlock(mutex_id: usize) -> isize {
    ...
    let mut task_inner = task.inner_exclusive_access();
    if process_inner.deadlock_detection {
        task_inner.mutex_locked.add(mutex_id, -1);
    }
...
pub fn sys_semaphore_up(sem_id: usize) -> isize {
    ...
    if process_inner.deadlock_detection {
        task_inner.semaphore_locked.add(sem_id, -1);
    }
    sem.up();
    0
}
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    let sem;
    {
        ...
        sem = Arc::clone(process_inner.semaphore_list[sem_id].as_ref().unwrap());
        {
            ...
            let mut task_inner = task.inner_exclusive_access();
            if process_inner.deadlock_detection {
                task_inner.semaphore_requesting.add(sem_id, 1);
            }
        }
        if process_inner.deadlock_detection {
            if process_inner.check_deadlock() {
...
pub fn sys_enable_deadlock_detect(enabled: usize) -> isize {
    ...
    let process = current_process();
    let mut process_inner = process.inner_exclusive_access();
    process_inner.deadlock_detection = match enabled {
        0 => false,
        1 => true,
        _ => { return -1; }
    };
