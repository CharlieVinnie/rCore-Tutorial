...
impl EasyFileSystem {
    ...
    pub fn dealloc_inode(&mut self, inode_id: u32) {
        ...
        let inode_data = get_block_cache(block_id as usize, Arc::clone(&self.block_device))
            .lock()
            .modify(block_offset, |disk_inode: &mut DiskInode| {
                // No deadlock because inodes and data are always in different blocks
                disk_inode.clear_size(&mut self.block_device)
            });
        self.inode_bitmap.dealloc(&self.block_device, inode_id as usize);
        for data_block_id in inode_data {
            self.dealloc_data(data_block_id);
        }
    }
